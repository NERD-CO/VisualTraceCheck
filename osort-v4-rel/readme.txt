...
See http://www.rutshauserlab.org/osort for details and updates.

version 4.10, released May 2018
(see below for a change log)

Copyright (c) 2005-2018 by Ueli Rutishauser. urut (---at---) caltech.edu
...
This release is tested with matlab version R2018a but older versions should work as well.

2. Neuralynx DLLs (windows) or mex files (linux/mac) to read Ncs files - these are included for convenience 
(code/3rdParty folder). The latest version can be downloaded from http://www.neuralynx.com/download.asp (Windows) or from http://www.rutishauserlab.org/nlxtools (Unix and MacOSX).
...
1. simulated data
2. binary data in manufacturer-specific formats. At present, this are neuralynx files (*.Ncs, both analog and digital cheetah versions).
3. text files
...
for the simulations without changing anything (See section 3.1. below).

Second, to get started run the sorting with the provided raw files as desctibed below (see section 3.2.).
...
1. modify sortingNew/model/loadSimulationFiles.m to point to the correct path for the datafiles (i.e. osort-v4-rel/data). You have to download the data files separately.
...
As part of osort, we provide demo data files which are recordings performed with microwires in an epilepsy patient. These files (CSC5.ncs, CSC6.ncs, CSC7.ncs)
are sampled at 32kHz and contain about 35min of data each.

To process these demo files, do the following:
1. modify osortTextUI/Standalone_textGUI_demo_new.m to adjust the path parameters. 
...
2. run osortTextUI/Standalone_textGUI_demo_new.m . The result will be stored in data/sort and data/figs
...
=============================================================
[the GUI has not been extensively tested for the new version - use the text version if it doesn't work]
...
[The tutorial has not been updated for v4, but is essentially still valid].
...
Raw data path: location of the raw data (for the example above,the path where CSC5.Ncs is located)
Raw files need to be called Ax.YYY, where x is the channel number and YYY the postfix. "A" is a prefix, which can be specified as a parameter.
...
4) How to get it running for your own data and postprocessing
=============================================================

To get an intuition for the algorithm and the evaluation plots it produces, try playing with the parameters in Standalone_textGUI_demo_new.m . This is the
...
4.1. Using the sorted spikes for your analysis
==============================================

For each sorted channel, OSort stores the result in a file Ax_sorted_new.mat with x being the channel number. This file contains all detected waveforms, their timestamps as well as
the cluster number that each waveform is assigned to. This is contained in the following variables:

newTimestampsNegative : timestamps, in us.
newSpikesNegative: 256 datapoints for each detected spike, at a sampling rate of 100kHz (upsampled from original).
assignedNegative: cluster number for each spike

The other variables contain additional information not necessary for spike analysis but useful to produce additional statistics or to reproduce a particular sorting run (parameters).
...
Format: 2 columns. 
First column: time in sample number (equal increases, no jumps). Usually this is just counting 1,2,3,..... Based on the sampling rate,this will be converted to time.
Second column: measured voltage (in uV).
...
Out of the box, the following data formats are support (see defineFileFormat.m): Neuralynx Analog, Neuralynx Cheetah, Neuralynx Atlas, Blackrock, Mat, txt.
See Standalone_textGUI_blackrock.m and convertNSx_toMat.m for how to use Blackrock files. The way Blackrock files are converted to intermediate Mat files can easily be adapted for most other systems, including AlphaOmega/NeuroOmega.

4.4 Post-processing
===================

See the following scripts for manual post processing (merging, selecting subsets of clusters as useful):
mergeClusters.m
defineUsableClusters.m
reassignToFixedClusters.m
clustersMinMaxMerge.m
...
Funding for OSort development has been provided by NIH (R01MH110831) and NSF (BCS-1554105).
...
v4.00 February 2018: release of various improvements and bugfixes from the development source tree. 
v4.10 May 2018: various adjustments to make v4 release work in Matlab 2018, bugfixes from the development source tree.
